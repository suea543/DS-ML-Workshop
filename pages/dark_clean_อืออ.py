...
st.set_page_config(page_title="🌌 Dark & Cool Data Cleaning", layout="wide", initial_sidebar_state="expanded")

st.title("🌌 Dark & Cool Data Cleaning Workshop App") # ตั้งชื่อแอปพลิเคชัน
st.markdown("ยินดีต้อนรับสู่แอปพลิเคชัน Data Cleaning ธีมดาร์คสุดเท่! 🚀") # ข้อความต้อนรับ
st.markdown("--- อัปโหลดไฟล์ CSV ของคุณและสำรวจพลังแห่งการทำความสะอาดข้อมูล! 🧽 ---\n") # คำแนะนำเบื้องต้น
st.error("⚠️ ใช้สำหรับชุดข้อมูลที่มีโครงสร้างเหมือน redbull_workshop_dirty.csv เท่านั้น")
...
if uploaded_file is not None: # ถ้ามีการอัปโหลดไฟล์แล้ว
    ...
    st.success("ไฟล์อัปโหลดสำเร็จ! ✅") # แสดงข้อความแจ้งว่าอัปโหลดสำเร็จ
    st.write("### 🗃️ Raw Data (5 แถวแรก)") # หัวข้อแสดงข้อมูลดิบ
    ...
    def perform_data_exploration(data): # ฟังก์ชันสำหรับสำรวจข้อมูลเบื้องต้น
        st.subheader("📊 1. Data Exploration: สำรวจข้อมูล") # หัวข้อย่อย
        st.write("#### 📐 Data Shape:") # หัวข้อย่อยแสดงขนาดข้อมูล
        st.write(f"จำนวนแถว: {data.shape[0]:,}, จำนวนคอลัมน์: {data.shape[1]}") # แสดงจำนวนแถวและคอลัมน์
        st.write("#### ℹ️ Data Info:") # หัวข้อย่อยแสดงข้อมูลทั่วไปของ DataFrame
        buffer = io.StringIO()
        ...
        st.markdown("---")
        st.write("#### 📊 การกระจายตัวของข้อมูลตัวเลข (ก่อนทำความสะอาด)")
    ...
    def handle_duplicate_data(data): # ฟังก์ชันสำหรับจัดการข้อมูลซ้ำ
        st.subheader("👥 2. Duplicate Data: จัดการข้อมูลซ้ำ") # หัวข้อย่อย
    ...
    def handle_inconsistent_data(data): # ฟังก์ชันสำหรับจัดการข้อมูลที่ไม่สอดคล้องกัน
        st.subheader("🔄 3. Inconsistent Data: แก้ไขข้อมูลไม่สอดคล้องกัน") # หัวข้อย่อย
        st.write("##### 🔍 ค่าที่ไม่สอดคล้องกัน (ก่อนแก้ไข):") # หัวข้อย่อยแสดงค่าก่อนแก้ไข
        ...
        st.write("##### 🧹 กำลังทำความสะอาดข้อความเบื้องต้น (ลบช่องว่าง, แปลงเป็นตัวพิมพ์เล็ก)...")
        for col in all_object_cols:
            if col not in ['Transaction_ID', 'Date']:
                data[col] = data[col].astype(str).str.strip().str.lower()

        st.write("##### 📊 Unique values หลังจากทำความสะอาดข้อความเบื้องต้น:")
        for col in cat_cols:
            unique_vals = data[col].unique()
            st.write(f"**📌 {col} ({len(unique_vals)} ค่า):**")
            st.write(unique_vals)

        st.write("##### 🛠️ กำลังแก้ไข Inconsistent Values เฉพาะคอลัมน์...") # ข้อความแจ้งว่ากำลังแก้ไข
        ...
        st.write("##### 📊 ค่าที่ไม่สอดคล้องกัน (หลังแก้ไข):") # หัวข้อย่อยแสดงค่าหลังแก้ไข
    ...
    def handle_missing_data(data): # ฟังก์ชันสำหรับจัดการข้อมูลที่หายไป (Missing Data)
        st.subheader("📭 4. Missing Data: จัดการค่าที่หายไป") # หัวข้อย่อย
        missing_count = data.isnull().sum() # นับจำนวนค่า Missing ในแต่ละคอลัมน์
        st.write("##### ⚠️ จำนวน Missing Values ก่อนแก้ไข:") # หัวข้อย่อยแสดงค่า Missing ก่อนแก้ไข
    ...
    def handle_noisy_data(data): # ฟังก์ชันสำหรับจัดการข้อมูลผิดพลาด (Noisy Data)
        st.subheader("📢 5. Noisy Data: ระบุและแก้ไขข้อมูลผิดพลาด") # หัวข้อย่อย
        st.write("##### 🕵️ ตรวจสอบ Business Logic ก่อนแก้ไข:") # หัวข้อย่อยแสดงการตรวจสอบ
    ...
    def perform_outlier_analysis(data): # ฟังก์ชันสำหรับตรวจจับ Outlier
        st.subheader("📐 6. Outlier Detection & Treatment: ตรวจจับและจัดการ Outlier") # หัวข้อย่อย
        ...
        if numeric_cols: # ถ้ามีคอลัมน์ตัวเลขให้วิเคราะห์ Outlier
            ...
            axes = axes.flatten() if n_rows > 1 or n_cols > 1 else [axes] # จัดการให้ axes เป็น array 1 มิติเสมอ
            for i, col in enumerate(numeric_cols): # วนลูปในแต่ละคอลัมน์ตัวเลข
                sns.boxplot(x=data[col], ax=axes[i], color='lightcoral') # สร้าง Boxplot
                axes[i].set_title(f'Boxplot of {col} (ก่อนปรับ)', fontsize=14) # ตั้งชื่อกราฟ
                axes[i].set_xlabel(col, fontsize=12) # ตั้งชื่อแกน X
            for j in range(i + 1, len(axes)): # ลบแกนว่างที่เหลือออก
                fig.delaxes(axes[j])
            plt.tight_layout() # ปรับระยะห่างระหว่าง subplots
            ...
            outlier_choice = st.radio(
                "เลือกวิธีการจัดการ Outlier สำหรับคอลัมน์ตัวเลข:",
                ('ไม่จัดการ Outlier 🙅', 'ใช้ Winsorization (ปรับค่าที่ปลาย) 🛡️'),
                key='outlier_treatment_radio'
            )

            if outlier_choice == 'ใช้ Winsorization (ปรับค่าที่ปลาย) 🛡️':
                st.write("##### ⚙️ กำลังปรับค่า Outlier ด้วย Winsorization...")
                for col in numeric_cols:
                    ...
                    st.success(f"✅ คอลัมน์ '{col}' ถูกปรับค่า Outlier ด้วย Winsorization (5%-95% percentile) แล้ว")

                st.write("##### 📊 Boxplot หลังการปรับค่า Outlier ด้วย Winsorization:")
                fig, axes = plt.subplots(n_rows, n_cols, figsize=(18, 5 * n_rows)) # สร้าง figure และ axes สำหรับ Boxplot หลังปรับ
                axes = axes.flatten() if n_rows > 1 or n_cols > 1 else [axes]
                for i, col in enumerate(numeric_cols):
                    sns.boxplot(x=data[col], ax=axes[i], color='mediumseagreen') # สร้าง Boxplot
                    axes[i].set_title(f'Boxplot of {col} (หลังปรับ Winsorization)', fontsize=14) # ตั้งชื่อกราฟ
                ...
                st.info("ไม่ได้ทำการจัดการ Outlier ใดๆ 🚫")
    ...
    do_outlier = st.sidebar.checkbox("6. Outlier Detection & Treatment 📐", value=True) # Checkbox สำหรับ Outlier Detection

    st.markdown("---  ") # เส้นแบ่ง

    if st.button("เริ่มกระบวนการทำความสะอาดข้อมูล 🚀"): # ปุ่มสำหรับเริ่มกระบวนการ Data Cleaning
        ...
        st.markdown("---") # เส้นแบ่ง
        st.subheader("✅ 7. Cleaned Data Summary: สรุปผล") # หัวข้อสรุปผล
        st.write(f"#### 📊 สรุปขนาดข้อมูล:") # หัวข้อย่อยแสดงขนาดข้อมูล
        st.write(f"ก่อนทำความสะอาด (Raw): {df_raw.shape[0]:,} แถว, {df_raw.shape[1]} คอลัมน์") # แสดงขนาดข้อมูลก่อนทำความสะอาด
        st.write(f"หลังทำความสะอาด (Clean): {df.shape[0]:,} แถว, {df.shape[1]} คอลัมน์") # แสดงขนาดข้อมูลหลังทำความสะอาด

        st.write("### 💎 Cleaned Data (5 แถวแรก)") # หัวข้อแสดงข้อมูลที่ทำความสะอาดแล้ว
        st.dataframe(df.head()) # แสดง 5 แถวแรกของข้อมูลที่ทำความสะอาดแล้ว

        st.markdown("---")
        st.write("#### 📈 การกระจายตัวของข้อมูลตัวเลข (หลังทำความสะอาด)")
        ...
        if numeric_cols_final: # ถ้ามีคอลัมน์ตัวเลขให้วิเคราะห์
            ...
            for i, col in enumerate(numeric_cols_final):
                sns.histplot(df[col].dropna(), kde=True, ax=axes[i], color='dodgerblue')
                axes[i].set_title(f'Distribution of {col} (หลังทำความสะอาด)', fontsize=14)
                axes[i].set_xlabel(col, fontsize=12)
            ...
            st.info("ไม่พบคอลัมน์ตัวเลขสำหรับการแสดงการกระจายตัวหลังทำความสะอาด")
        ...
        st.download_button( # ปุ่มสำหรับดาวน์โหลดข้อมูล
            label="ดาวน์โหลดข้อมูลที่ทำความสะอาดแล้ว (CSV) 💾", # ข้อความบนปุ่ม
            data=csv_buffer, # ข้อมูลที่จะให้ดาวน์โหลด
            file_name="redbull_clean_dark_theme.csv", # ชื่อไฟล์เมื่อดาวน์โหลด
