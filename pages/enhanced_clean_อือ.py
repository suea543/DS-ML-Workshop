...
# --- Streamlit App Title ---
st.title("🐂 Enhanced Data Cleaning Workshop App") # ตั้งชื่อแอปพลิเคชัน
st.markdown("ยินดีต้อนรับสู่แอปพลิเคชัน Data Cleaning ที่ได้รับการปรับปรุง!") # ข้อความต้อนรับ
...
if uploaded_file is not None: # ถ้ามีการอัปโหลดไฟล์แล้ว
    ...
    def handle_inconsistent_data(data): # ฟังก์ชันสำหรับจัดการข้อมูลที่ไม่สอดคล้องกัน
        ...
        all_object_cols = data.select_dtypes(include='object').columns.tolist()

        # General standardization for all object columns (strip spaces and lowercase)
        ...
        # Display unique values after general cleaning, before specific mappings
        ...
        st.write("##### กำลังแก้ไข Inconsistent Values เฉพาะคอลัมน์...") # ข้อความแจ้งว่ากำลังแก้ไข
        ...
        # Ensure consistent casing for product variants that might not be in mapping but are now lowercase
    ...
    def perform_outlier_analysis(data): # ฟังก์ชันสำหรับตรวจจับ Outlier
        ...
        if numeric_cols: # ถ้ามีคอลัมน์ตัวเลขให้วิเคราะห์ Outlier
            ...
            outlier_choice = st.radio(
                "เลือกวิธีการจัดการ Outlier (สำหรับคอลัมน์ตัวเลขที่แสดงด้านบน):",
                ('ไม่ทำอะไร', 'ใช้ Winsorization (ปรับค่าที่ปลาย)'),
                key=f'outlier_treatment_radio'
            )

            if outlier_choice == 'ใช้ Winsorization (ปรับค่าที่ปลาย)':
                st.write("##### กำลังปรับค่า Outlier ด้วย Winsorization...")
                ...
                for col in numeric_cols: # Re-plot boxplots after treatment
                    fig, ax = plt.subplots(figsize=(8, 2))
                    sns.boxplot(x=data[col], ax=ax)
                    ax.set_title(f'Boxplot of {col} (After Winsorization)')
                    st.pyplot(fig)
                    plt.close(fig)
            else: # 'ไม่ทำอะไร'
